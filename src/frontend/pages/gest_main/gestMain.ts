...
async function loadDashboard() {
    try {
        ...
        if (totalUsersEl) totalUsersEl.textContent = (usersData.data?.length || 0).toString();

        const totalProductsEl = document.getElementById("totalProducts");
        if (totalProductsEl) totalProductsEl.textContent = (productsData.data?.length || 0).toString();

        const totalTablesEl = document.getElementById("totalTables");
        if (totalTablesEl) totalTablesEl.textContent = (tablesData.data?.length || 0).toString();

        const openOrders = ordersData.data?.filter((o) => o.status === "OPEN") || [];
        ...
        if (availableTablesEl) availableTablesEl.textContent = availableTables.toString();

        const occupiedTablesEl = document.getElementById("occupiedTables");
        if (occupiedTablesEl) occupiedTablesEl.textContent = occupiedTables.toString();

        const reservedTablesEl = document.getElementById("reservedTables");
        if (reservedTablesEl) reservedTablesEl.textContent = reservedTables.toString();
...
function renderRecentOrders(ordersData: Order[]) {
    ...
    if (recentOrders.length === 0) {
        container.innerHTML = '<p class="loading-text">Nenhum pedido encontrado</p>';
        return;
    }
...
function renderUsers(usersData: User[]) {
    ...
    if (usersData.length === 0) {
        tbody.innerHTML = '<tr><td colspan="5" class="loading-cell">Nenhum usuário encontrado</td></tr>';
        return;
    }
...
function showUserForm(userId: string | null = null) {
    ...
    if (modalTitle) modalTitle.textContent = isEdit ? "Editar Usuário" : "Novo Usuário";
    if (modalBody) {
        modalBody.innerHTML = `
...
function showProductForm(productId: string | null = null) {
    ...
    if (modalTitle) modalTitle.textContent = isEdit ? "Editar Produto" : "Novo Produto";
    if (modalBody) {
        modalBody.innerHTML = `
...
function renderOrders(ordersData: Order[]) {
    ...
    if (ordersData.length === 0) {
        tbody.innerHTML = '<tr><td colspan="6" class="loading-cell">Nenhum pedido encontrado</td></tr>';
        return;
    }
...
async function viewOrder(orderId: string) {
    try {
        ...
        if (modalTitle) modalTitle.textContent = `Pedido #${orderId.substring(0, 8)}`;
        if (modalBody) {
            modalBody.innerHTML = `
...
function showCategoryForm(categoryId: string | null = null) {
    ...
    if (modalTitle) modalTitle.textContent = isEdit ? "Editar Categoria" : "Nova Categoria";
    if (modalBody) {
        modalBody.innerHTML = `
...
function applyProductFilters() {
    ...
    const statusInput = document.getElementById("statusFilter") as HTMLSelectElement;
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.getElementById("addTableBtn")?.addEventListener("click", () => showTableForm());
    ...
    document.getElementById("searchProducts")?.addEventListener("input", applyProductFilters);
    ...
    document.getElementById("tableStatusFilter")?.addEventListener("change", (e) => {
        ...
        const filtered = status ? tables.filter((t) => t.status === status) : tables;
