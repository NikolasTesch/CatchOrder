      ...
      order.items.forEach((item: any) => {
        if (item.name) {
          productStats[item.name] =
            (productStats[item.name] || 0) + (item.quantity || 0);
        }
      });
    }
  });
    ...
    popover.innerHTML = `
      ...
      <div class="popover-body">
        <div class="notification-empty">
          <span class="material-symbols-outlined" style="font-size: 48px; color: #ccc; display: block; margin-bottom: 10px;">notifications_off</span>
...
function setupUserFormValidation(isEdit: boolean) {
  ...
  const validatePassword = () => {
    if (!passwordInput) return; // In edit mode, password might be optional/hidden or handled differently
    const value = passwordInput.value;

    // If edit mode and empty, it might be valid (keep current password).
    // But the form template for edit mode doesn't include password input currently (lines 641-650 in original).
    ...
    .map((product) => {
      ...
      <div class="product-card">
        <div class="product-header">
          <div class="product-icon" style="background-image: url('${product.image_path ? resolveImagePath(product.image_path) : ''}'); background-size: cover; background-position: center;">
            ${!product.image_path ? '<span class="material-symbols-outlined">restaurant</span>' : ''}
          </div>
          <div>
...
async function fetchInsightsData() {
  try {
    ...
    const result = await apiCall<{ data: any }>(
      `/metrics/insights?startDate=${startISO}&endDate=${endISO}`,
    );
...
function renderWaiterTable(waiters: any[]) {
  ...
  if (waiters.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="4" class="empty-text">Nenhum dado no período.</td></tr>';
    return;
  }
