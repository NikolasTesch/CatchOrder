...
document.addEventListener('DOMContentLoaded', () => {
    const productsGrid = document.getElementById('products-grid') as HTMLElement | null;
    const searchInput = document.querySelector('.search-input') as HTMLInputElement | null;
    const btnAddProduct = document.querySelector('.btn-add-product') as HTMLButtonElement | null;
    const btnAddCard = document.getElementById('btn-add-product') as HTMLElement | null; // The card button
    ...
    function renderProducts(items: Product[]): void {
        ...
        addCard.innerHTML = '<span class="material-symbols-outlined add-icon">add</span>';
    ...
    function createProductCard(product: Product): HTMLElement {
        ...
        const bgImage = product.image_path ? `url('${product.image_path}')` : 'none';
      ...
      <div class="card-info">
        ...
        <span class="product-category">R$ ${product.price.toFixed(2)}</span>
      </div>
    `;
        return card;
    }
